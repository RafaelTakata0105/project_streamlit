...
st.subheader('Haz tus propias gráficas de las clases:')
df_graph = st.selectbox('Selecciona el Dataframe que quieres graficar:', [porclass_df, math_df])
graf_col = st.selectbox('Selecciona las columnas a graficar:', porclass_df.columns)
fig, ax = plt.subplots()
ax.hist(df_graph[graf_col], bins=20, alpha=0.7, color='blue')
